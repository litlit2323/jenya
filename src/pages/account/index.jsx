...
const NoDataIndication = () => {
  return (
    <p>
      Ваш список заказов до сих пор. Вы можете сделать его{' '}
...
const Account = ({ user }) => {
  ...
  const removeHandler = () => {
    const removeConfirm = confirm(
      'Вы уверены, что хотите отказаться от указанных заказов?'
    )

    if (removeConfirm && node.selectionContext.selected) {
      axios.post(
        '/api/user/orders/deleteOrders',
        { recordsToDelete: node.selectionContext.selected },
        { withCredentials: true }
      )
      setOrders(orders => {
        return orders.filter(
          order => !node.selectionContext.selected.includes(order._id)
        )
      })
    }
  }
  ...
  const rowEvents = {
    onClick: (e, row, rowIndex) => {
      router.push({
        pathname: `/account/orders/${row._id}`,
        query: { id: row._id },
      })
    },
  }
        ...
        <Row>
          <Col md={4}>
            <Container fluid>
              <Card>
                <Card.Header as={'h1'}>
                  {userAccount?.secondName} {userAccount?.firstName}{' '}
                  {userAccount?.patronymicName}
                </Card.Header>
                <ListGroup variant='flush'>
                  <ListGroup.Item>E-mail: {userAccount?.email}</ListGroup.Item>
                  <ListGroup.Item>
                    Телефон: {userAccount?.phoneNumber}
                  </ListGroup.Item>
                ...
                <Row className={'justify-content-end pt-3 px-3'}>
                  <Col sm={4}>
                    <Button onClick={removeHandler}>
                      Отказаться от выделенных заказов
                    </Button>
                  </Col>
                </Row>
